...
global p_file,detector,predictor
...
line, = ax.plot(x_pts, z_pts, y_pts, '-r')
ax.set_ylabel("z")
ax.set_xlabel("x")
ax.set_zlabel("y")
...
def run(dt):
    global fig, ax, x_pts, y_pts, z_pts, profile
    ...
    if(len (x_pts)== 1):
        plt.ion()
        line, = ax.plot(x_pts, y_pts, z_pts, '-r', alpha=0.8)
    ...
    for (i, rect) in enumerate(rects):
        ...
        nose_pixel = shape[30] # tip of nose
        nose_depth = depth_data.get_distance(nose_pixel[0], nose_pixel[1]) #depth in meters
        nose = [nose_pixel[0], nose_pixel[1]]

        depth_intrin = depth_frame.profile.as_video_stream_profile().intrinsics
        depth_point = rs.rs2_deproject_pixel_to_point(depth_intrin, [nose_pixel[0], nose_pixel[1]], nose_depth)
        test_point = rs.rs2_deproject_pixel_to_point(depth_intrin, [shape[29][0], shape[29][1]], depth_data.get_distance(shape[29][0], shape[29][1]))


        x_pts = np.append(x_pts, depth_point[0])
        y_pts = np.append(y_pts, depth_point[1])
        z_pts = np.append(z_pts, nose_depth)

        draw_graph()


        cv2.circle(color_image, (nose_pixel[0], nose_pixel[1]), 5, (0, 255, 0), -1)

        cv2.circle(color_image, (shape[29][0], shape[29][1]), 5, (0, 0, 255), -1)
...
def draw_graph():
    global fig, ax, x_pts, y_pts, z_pts, line
    # line.set_xdata(x_pts)
    # line.set_ydata(y_pts)
    line.set_data(x_pts, z_pts)
    line.set_3d_properties(y_pts)
    ...
    # line, = ax.plot(x_pts, y_pts, z_pts, '-r')
    # plt.draw()
